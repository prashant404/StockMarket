...
import { fetchUserDetails, fetchWatchlist, removeFromPortfolio } from '../api/stockAPI';
...
const PortfolioPage = () => {
  ...
  const handleRemoveFromPortfolio = async (stockId) => {
    try {
      await removeFromPortfolio(stockId);
  ...
  return (
    <Container>
      ...
      <Box>
        ...
        <Heading size="md" mt={4}>Stocks Added to Watchlist</Heading>
        {watchlist.length > 0 ? (
          <SimpleGrid columns={[1, 2, 3]} spacing={4} mt={4}>
            {watchlist.map((stock) => (
              <Box key={stock._id} borderWidth="1px" borderRadius="lg" overflow="hidden" p={4}>
                ...
                <Button mt={2} colorScheme="red" onClick={() => handleRemoveFromPortfolio(stock._id)}>
          ...
          <Text>No stocks added in watchlist.</Text>
        )}
      </Box>
    </Container>
  );
